//! Model struct for SearchAvailabilityFilter type
...
pub struct SearchAvailabilityFilter {
    /// The query expression to search for buy-accessible availabilities with their starting times
    /// falling within the specified time range. The time range must be at least 24 hours and at
    /// most 32 days long. For waitlist availabilities, the time range can be 0 or more up to 367
    /// days long.
    ...
    /// The query expression to search for buyer-accessible availabilities for an existing booking
    /// by matching the specified booking_id value. This is commonly used to reschedule an
